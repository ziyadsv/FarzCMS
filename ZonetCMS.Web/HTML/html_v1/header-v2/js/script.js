;(function($){
	...
	if($opNavLink.length > 0 ){
		$opNavLink.on('click', function (e) {
			if (location.pathname.replace(/^\//,'') === this.pathname.replace(/^\//,'') || location.hostname === this.hostname) {
				var goto = $(this.hash), nbar = (wwCurrent >= 992) ? $(".navbar").height() - 10 : 0;
				goto = goto.length ? goto : $('[name=' + this.hash.slice(1) +']');
				
				if (goto.length) {
					$('html,body').animate({
						scrollTop: goto.offset().top - nbar
					}, 1200);
				}
				if (wwCurrent < 768) {
